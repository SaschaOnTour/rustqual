...
- **Trait-method anchors** — synthetic `<Trait>::<method>` nodes emitted by `dyn Trait.method()` dispatch, recognised as a boundary when (0) the trait declaration is workspace-visible per the shared `visible_canonicals` set — covering `pub`, `pub(crate)`, `pub(super)`, `pub(in <path>)`, file-backed module visibility, public re-exports, and inline-mod visibility chains; private traits and pub-traits-inside-private-mods are implementation detail and excluded, AND (a) the trait declaration is NOT in a peer-adapter layer, AND (b) the trait declaration IS in the target layer with a callable body (default OR overriding impl), OR at least one overriding impl lives in the target layer. Inherited-default impls in target (`impl T for X {}` where `T::method` has its default body declared outside target) do NOT promote — the body lives at the trait's declaring layer and isn't modelled as a target node. The anchor itself can live in any non-peer-adapter layer (typically `ports` for Hexagonal, or `application` itself for in-target traits). Check B/D enumerate anchors alongside concrete target pub-fns when computing coverage and multiplicity, and skip concrete impl-method pub-fns that an enumerated anchor already covers — **but only when no adapter has the concrete in its boundary coverage**. When an adapter reaches the capability via direct concrete call (`LoggingHandler::handle()` UFCS or static-method form) and another via `dyn Trait` dispatch, the concrete pass still runs so mixed-form drift surfaces as paired concrete + anchor findings (cross-form synonym handling stays intentionally out of scope). When ALL adapters cover the capability via direct concrete call (no dispatch anywhere), the anchor pass is silenced — the capability IS covered, just via the concrete form. `exclude_targets` globs match against both the anchor canonical and any `impl_method_canonical` it backs, so a single user-friendly impl-path glob (`application::admin::*`) silences both the concrete and anchor forms without needing a parallel ports-path entry. Anchor findings carry the trait method's actual declaration line so suppression markers and SARIF locations work correctly.
...
- Trait dispatch on `dyn Trait` and `impl Trait` collapses to a single synthetic anchor `<Trait>::<method>` representing the logical capability — the call graph **does not** fan out to per-impl edges, which would otherwise turn one boundary call into N touchpoints and falsely fire Check C. The walker recognises the anchor as a target boundary when (a) the trait is declared in the target layer with a callable body (default OR overriding impl), OR (b) at least one overriding impl lives in the target layer. Inherited-default impls (empty `impl T for X {}` whose body lives on the trait elsewhere) do not promote — the body isn't modelled as a target node.
