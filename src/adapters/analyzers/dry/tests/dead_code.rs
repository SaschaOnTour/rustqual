...
fn a_re_export_is_exposure_not_consumption() {
    ...
    for (label, code) in REEXPORT_ONLY {
        ...
        assert!(
            names.contains(&"do_work"),
            "{label}: re-exported but never called is dead: {found:?}"
        );
        assert!(
            *label != "grouped" || names.contains(&"other"),
            "{label}: every name in the group: {found:?}"
        );
    }
}

/// Each way a `use` can expose a function, followed by consumption *through*
/// what it exposed. The claim "a use is exposure, not consumption" rests on the
/// other half — that consumption through the exposed name still counts — and
/// that half is the one a bare-name call set can get wrong: under a rename the
/// call site says the alias, and the declaration's own name is never spoken.
const CONSUMED_THROUGH_EXPOSURE: &[(&str, &str)] = &[
    (
        "plain re-export",
        "mod foo { pub fn do_work() { let x = 1; } }\npub use foo::do_work;\nfn main() { do_work(); }",
    ),
    (
        "renamed re-export",
        "mod foo { pub fn do_work() { let x = 1; } }\npub use foo::do_work as w;\nfn main() { w(); }",
    ),
    (
        "renamed private import",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse foo::do_work as w;\nfn main() { w(); }",
    ),
    (
        "glob re-export",
        "mod foo { pub fn do_work() { let x = 1; } }\npub use foo::*;\nfn main() { do_work(); }",
    ...
    (
        "import inside a body",
        "mod foo { pub fn do_work() { let x = 1; } }\nfn main() { use foo::do_work; do_work(); }",
    ),
    (
        "alias of an alias",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse foo::do_work as a;\nuse a as b;\nfn main() { b(); }",
    ),
    (
        "renamed import inside a body",
        "mod foo { pub fn do_work() { let x = 1; } }\nfn main() { use foo::do_work as w; w(); }",
    ),
    // A binding made by a `use` in module `m` is reached as `m::name` from
    // anywhere, through the path's *last* segment — the ordinary facade shape.
    (
        "renamed re-export called through its facade module",
        "mod foo { pub fn do_work() { let x = 1; } }\nmod facade { pub use crate::foo::do_work as w; }\nfn main() { facade::w(); }",
    ),
    (
        "renamed import called from a child module",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse foo::do_work as w;\nmod child { pub fn go() { super::w(); } }\nfn main() { child::go(); }",
    ),
    (
        "renamed re-export called through the crate root",
        "mod foo { pub fn do_work() { let x = 1; } }\npub use foo::do_work as w;\nmod m { pub fn go() { crate::w(); } }\nfn main() { m::go(); }",
    ),
    (
        "renamed import called from a grandchild module",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse foo::do_work as w;\nmod a { pub mod b { pub fn go() { super::super::w(); } } }\nfn main() { a::b::go(); }",
    ),
    (
        "renamed re-export called through an alias of the crate root",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse crate as API;\npub(crate) use foo::do_work as w;\nfn main() { API::w(); }",
    ),
    (
        "renamed re-export called through an alias of an external crate",
        "mod foo { pub fn do_work() { let x = 1; } }\nextern crate dep as API;\npub(crate) use foo::do_work as w;\nfn main() { API::w(); }",
    ),
    (
        "renamed re-export called through an alias of a CamelCase external crate",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse Dep as API;\npub(crate) use foo::do_work as w;\nfn main() { API::w(); }",
    ),
    (
        "renamed re-export generated by a macro_rules! transcriber",
        "mod foo { pub fn do_work() { let x = 1; } }\n\
        macro_rules! expose { () => { mod api { pub use crate::foo::do_work as w; } }; }\nexpose!();\n\
        fn main() { api::w(); }",
    ),
    (
        "renamed import called through self::",
        "mod foo { pub fn do_work() { let x = 1; } }\nuse foo::do_work as w;\nfn main() { self::w(); }",
    ),
    (
        "renamed re-export as a struct field value",
        "mod foo { pub fn do_work() { let x = 1; } }\npub use foo::do_work as w;\nstruct Cfg { f: fn() }\nfn main() { let _ = Cfg { f: crate::w }; }",
    ),
];

#[test]
fn a_function_consumed_through_what_exposes_it_is_alive() {
    for (label, code) in CONSUMED_THROUGH_EXPOSURE {
        let found = dead_code_warnings(&parse(code));
        assert!(
...
fn two_renames_to_one_alias_keep_both_originals() {
    // `use x as run` in one module and `use y as run` in another is ordinary
    // Rust. A map keyed by the alias kept whichever came last and reported the
    // other original dead — and which one lost depended on walk order.
    let code = "mod w { pub fn work() { let x = 1; } pub fn other_work() { let x = 1; } }\n\
        mod one { use crate::w::work as run; pub fn a() { run(); } }\n\
        mod two { use crate::w::other_work as run; pub fn b() { run(); } }\n\
        fn main() { one::a(); two::b(); }";
    let found = dead_code_warnings(&parse(code));
    let names: Vec<&str> = found.iter().map(|w| w.function_name.as_str()).collect();
    assert!(
        !names.contains(&"work") && !names.contains(&"other_work"),
        "{found:?}"
    );
}

#[test]
fn a_rename_in_one_file_serves_a_call_site_in_another() {
    let parsed = parse2(
        "src/lib.rs",
        "mod inner;\npub use inner::work as perform;\nfn main() { inner::go(); }",
        "src/inner.rs",
        "pub fn work() { let x = 1; }\nuse crate::perform;\npub fn go() { perform(); }",
    );
    let found = dead_code_warnings(&parsed);
    assert!(
        !found.iter().any(|w| w.function_name == "work"),
        "{found:?}"
    );
}

/// The verdict that `qual:test_helper` excuses is `testonly`, never
/// `uncalled` — and never nothing at all. Each shape here has to land on
/// exactly that verdict for `helper`, or the marker silently stops working for
/// the code it was written for.
const TEST_ONLY_THROUGH_AN_ALIAS: &[(&str, &str)] = &[
    // A test module imports the helper under another name and calls it by
    // that name: `uncalled` would be the false positive.
    (
        "test alias, called only from the test",
        "pub fn helper() { let x = 1; }\n#[cfg(test)]\nmod tests { pub use super::helper as h; fn t() { h(); } }",
    ),
    // The alias `h` exists only inside the test module, but production calls
    // its own `h`. A context-blind alias map turned that production call into
    // a production call of `helper` — no finding, so the marker read as spent.
    (
        "test alias colliding with a production name",
        "pub fn helper() { let x = 1; }\npub fn h() { let x = 1; }\nfn main() { h(); }\n\
        #[cfg(test)]\nmod tests { use super::helper as h; fn t() { h(); } }",
    ),
    // The alias sits one module up from the caller, which reaches it as
    // `super::h()`: the last segment is the head.
    (
        "test alias reached through super::",
        "pub fn helper() { let x = 1; }\n\
        #[cfg(test)]\nmod tests { use super::helper as h; mod deeper { fn t() { super::h(); } } }",
    ),
];

#[test]
fn a_helper_reached_only_through_a_test_alias_is_test_only() {
    for (label, code) in TEST_ONLY_THROUGH_AN_ALIAS {
        let found = dead_code_warnings(&parse(code));
        let verdict = found
            .iter()
            .find(|w| w.function_name == "helper")
            .map(|w| w.kind.clone());
        assert_eq!(verdict, Some(DeadCodeKind::TestOnly), "{label}: {found:?}");
    }
}
...
fn an_ordinary_macro_invocation_does_not_vouch_for_its_arguments() {
    ...
    // that trigger, `name_of!(dead_helper)` — a macro that only spells its
    // argument — would mark a plainly dead function as called, which is the
    // mistake that costs a real finding.
    let code = r#"
macro_rules! name_of { ($f:ident) => { stringify!($f) }; }
fn dead_helper() -> u8 { 1 }
fn used() { let x = name_of!(dead_helper); assert_eq!(x, "dead_helper"); }
fn main() { used(); }
"#;
    let found = dead_code_warnings(&parse(code));
...
"#,
    ...
    (
        (
            // `wrapper!` applies `$a` itself and hands `$b` to `end!`, which
            // applies it. The closure recognised `wrapper` in the first round —
            // before `end` was known — and never looked at it again, so the
            // forwarded position was lost.
            "a macro that both applies and forwards",
            r#"
macro_rules! end { ($f:path) => { $f(); }; }
macro_rules! wrapper { ($a:path, $b:path) => { $a(); end!($b); }; }
fn first() {}
fn second() {}
fn used() { wrapper!(first, second); }
fn main() { used(); }
"#,
            &[],
        ),
    ...
    (
        (
            // `($f)()` is a call as much as `$f()` is; the scan only looked
            // for the argument group directly after the metavariable.
            "a parenthesised callee",
            r#"
...
"#,
    ...
    (
        (
            // Two `go!` macros in two modules, applying different positions.
            // Which one an invocation reaches is textual scope rustqual does
            // not model; keying by name let the last definition answer for
            // both, and one really-called function was reported dead. Under a
            // name defined twice the shape is undecidable, and undecidable
            // means every argument is a possible callee.
            "a macro name defined twice",
            r#"
mod one {
    macro_rules! go { ($f:path, $g:path) => { $f(); }; }
    fn live_a() {}
    fn other_a() {}
    pub fn x() { go!(live_a, other_a); }
}
mod two {
    macro_rules! go { ($f:path, $g:path) => { $g(); }; }
    fn other_b() {}
    fn live_b() {}
    pub fn y() { go!(other_b, live_b); }
}
fn main() { one::x(); two::y(); }
"#,
            &[],
    ...
    (
        (
            // Two macros forwarding to each other, with rules of different
            // arity: `p!(w,x,y,z)` → `q!(x,y,w)` → `p!(y,x)` → `q!(x)` → `x()`.
            // Positions spread one hop per round, and the chain here is longer
            // than the number of macros — a round count bounded by that number
            // stopped early and reported `x` dead. The loop runs to a fixpoint.
            "a forwarding chain longer than the macro count",
            r#"
macro_rules! p {
    ($a:ident, $b:ident, $c:ident, $d:ident) => { q!($b, $c, $a) };
    ($a:ident, $b:ident) => { q!($b) };
}
macro_rules! q {
    ($a:ident, $b:ident, $c:ident) => { p!($b, $a) };
    ($a:ident) => { $a() };
}
fn w() {}
fn x() {}
...
"#,
    ...
    (
        (
            // A name defined twice is undecidable only once one definition
            // calls through. Neither does here, so neither invocation vouches
            // for its argument — the `assert_eq!(x, dead)` shape, twice.
            "a macro name defined twice, neither calling through",
            r#"
mod one {
    macro_rules! go { ($f:path) => { let _ = stringify!($f); }; }
    fn dead_a() {}
    pub fn x() { go!(dead_a); }
}
mod two {
    macro_rules! go { ($f:path) => { let _ = 1; }; }
    fn dead_b() {}
    pub fn y() { go!(dead_b); }
}
fn main() { one::x(); two::y(); }
"#,
    ...
    (
        (
            // The parenthesised callee is still text inside a quoting macro,
            // and `($f) { … }` is not a call at all.
            "a parenthesised metavariable that is not called",
            r#"
macro_rules! quoted { ($f:path) => { let _ = stringify!(($f)()); }; }
macro_rules! braced { ($f:path) => { let _ = ($f) {}; }; }
fn dead_quoted() {}
fn dead_braced() {}
fn used() { quoted!(dead_quoted); braced!(dead_braced); }
fn main() { used(); }
"#,
            &["dead_quoted", "dead_braced"],
        ),
        &[],
    ),
];
...
#[test]
fn a_qualified_call_may_reach_a_binding_in_its_module_known_limit() {
    // `m::perform()` calls `m`'s own function; the crate-root rename
    // `use n::work as perform` is unrelated. But `m::perform` would look the
    // same if `m` held a `pub use n::work as perform`, and by bare name the
    // two are one — so the last segment of a module-qualified path is a head,
    // and `work` stays alive. The limit errs toward a missed finding.
    let code =
        "mod m { pub fn perform() { let x = 1; } }\nmod n { pub fn work() { let x = 1; } }\n\
        use n::work as perform;\nfn main() { m::perform(); }";
    let found = dead_code_warnings(&parse(code));
    assert!(
        !found.iter().any(|w| w.function_name == "work"),
        "{found:?}"
...
#[test]
fn a_module_qualifier_is_not_a_call_of_a_same_named_function() {
    // `facade::perform()` calls `perform`; the `facade` in front of it is a
    // module, and a function can never sit there. Recording the qualifier as
    // a call hid a dead `fn facade` — and, worse, made TQ-003 report it as an
    // untested function with production callers.
    let code = "mod facade { pub fn perform() { let x = 1; } }\nfn facade() { let x = 1; }\n\
        fn main() { facade::perform(); }";
    let found = dead_code_warnings(&parse(code));
    assert!(
        found.iter().any(|w| w.function_name == "facade"),
        "{found:?}"
    );
}
...
fn a_binding_a_metavariable_would_create_is_not_seen_known_limit() {
    ...
    // that `expose!(Alias)` would bind never exists for the analysis, so
    // `Alias()` reaches nothing and `real` reads as uncalled. Code a macro
    // *generates* is outside the grain — like `include!` and proc macros —
    // and `qual:api` on `real` is the documented answer.
    let code = "pub fn real() { let x = 1; }\n\
        macro_rules! expose { ($name:ident) => { pub use crate::real as $name; }; }\nexpose!(Alias);\n\
        fn main() { Alias(); }";
    let found = dead_code_warnings(&parse(code));
    assert!(found.iter().any(|w| w.function_name == "real"), "{found:?}");
}

#[test]
fn a_use_of_a_local_type_may_be_a_use_of_a_crate_known_limit() {
    // `struct lower; use lower as API;` — here the alias names the type, and
    // `API::perform()` is an associated function, so rustc has `work` unused.
    // But `use lower as API` reads the same when `lower` is an external
    // crate and the struct sits in another module, and the head rule cannot
    // tell the two apart, so the ambiguity goes to *module*: `perform` is a
    // head, the unrelated rename is consumed, `work` stays alive. A missed
    // finding; TQ-003 does not inherit it, since it reads the unwidened set.
    let code = "struct lower;\nimpl lower { pub fn perform() { let x = 1; } }\nuse lower as API;\n\
        mod other { pub fn work() { let x = 1; } }\nuse other::work as perform;\nfn main() { API::perform(); }";
    let found = dead_code_warnings(&parse(code));
    assert!(
        !found.iter().any(|w| w.function_name == "work"),
        "{found:?}"
...
#[test]
fn an_associated_function_is_not_a_renamed_import() {
    // `Type::perform()` is an associated function; nothing a `use` bound can
    // sit behind a CamelCase qualifier, so the rename stays unconsumed.
    let code = "struct Type;\nimpl Type { pub fn perform() { let x = 1; } }\n\
        mod n { pub fn work() { let x = 1; } }\nuse n::work as perform;\nfn main() { Type::perform(); }";
    let found = dead_code_warnings(&parse(code));
    assert!(found.iter().any(|w| w.function_name == "work"), "{found:?}");
}

#[test]
fn a_rename_serves_every_caller_of_its_name_known_limit() {
    // Renames resolve by bare name, the grain of the whole call graph: `use
    // inner::dead as run` in one module and a call to a *different* `run` in
    // another pool, exactly as two functions named `run` already do. Scoping
    // this needs a resolver that knows which `run` a call site sees, which
    // rustqual does not have. The limit errs toward a missed finding.
    let code = "mod inner { pub fn dead() { let x = 1; } }\n\
        mod a { use crate::inner::dead as run; }\n\
        mod b { fn run() { let x = 1; } pub fn go() { run(); } }\n\
        fn main() { b::go(); }";
    let found = dead_code_warnings(&parse(code));
    assert!(
        !found.iter().any(|w| w.function_name == "dead"),
        "{found:?}"
    );
}
