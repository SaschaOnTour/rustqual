...
pub fn detect_dead_code(
    ...
    // alive; what a `use` implies once its binding is consumed is resolved by
    // `widened`. See `call_targets::visit_item_use`.
    let calls = collect_all_calls(parsed, cfg_test_files).widened();
