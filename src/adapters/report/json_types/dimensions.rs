//! Per-dimension JSON payload types. Plain DTOs: field names are the wire
//! format, so a rename here is a breaking change for every consumer.
...
pub(crate) struct JsonTqWarning {
    ...
    pub(crate) function_name: String,
    pub(crate) kind: String,
    /// How *this* finding was answered — see `domain::findings::CoverageEvidence`.
    /// The summary's `coverage` describes the run, not this function.
