...
pub(super) fn sarif_rules() -> Vec<serde_json::Value> {
    vec![
        rule("iosp/violation", "Function violates the Integration Operation Segregation Principle"),
        ...
        rule("SRP-001", "Struct may violate Single Responsibility Principle"),
        rule("SRP-002", "Module has excessive production code length"),
        rule("SRP-003", "Function has too many parameters — reduce parameter count"),
        ...
        rule("TQ-002", "Test function does not call any production function"),
        ...
        rule("NMS", "Needless &mut self: takes &mut self but never mutates"),
        rule("OI", "Orphaned impl: impl block in different file than type"),
        rule("SIT", "Single-impl trait: non-pub trait with exactly one implementation"),
...
fn rule(id: &str, description: &str) -> serde_json::Value {
    ...
    if id == "iosp/violation" {
        entry.as_object_mut()
            ...
            .insert("helpUri".to_string(), serde_json::json!("https://flow-design.info/"));
    }
    entry
}
